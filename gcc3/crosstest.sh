...
# Remove test failures observed to be intermittantly caused by dejagnu truncating compiler output in local_exec
# (bug in expect?)
fgrep -v -f $TOP_DIR/dejagnu-bug < test_summary.log > $TOP_DIR/$TARGET-$GCC_DIR-$GLIBC_DIR.gcc.sum2
