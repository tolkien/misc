...
#########################################################
# <프로그램 4-5> 우리나라 경기동행지수 순환변동치의 선그래프 작성   #
#########################################################

library(ggplot2)
library(scales)
# 경기종합지수 데이터 불러와서 xts 객체로 만듬
cycle1  <- read.csv("i:/R/datavis/cycle.csv",header=TRUE)
연도    <- seq(as.Date("1970-01-01"), as.Date("2014-06-01"), "month")
cycle   <- xts(cycle1[,2],연도)
# 경기 기준순환일 데이터를 가져와서 정리
refdate <- read.csv("i:/R/datavis/refdate1.csv",header=TRUE)
yrng <- range(cycle)
...
######################################################### 
# <프로그램 4-6>  제주시 강수량의 선그래프 작성                            #
######################################################### 

library(ggplot2)
# 데이터 불러오기
climate_kr1  <- read.csv("i:/R/datavis/climate.csv", header=TRUE)
...
######################################################### 
# <프로그램 4-7>       강릉시, 서울시, 제주시 기온의 선그래프 작성      #
######################################################### 

library(ggplot2)
# 데이터 불러오기
climate_kr1  <- read.csv("i:/R/datavis/climate1.csv", header=TRUE)
연도         <- rep(seq(as.Date("2008-01-01"), as.Date("2013-12-01"), "month"),3)
climate_kr   <- cbind(연도, climate_kr1)
# 그래프 그리기
h <- ggplot(climate_kr, aes(x=연도))+facet_grid(지역 ~.) +
      geom_ribbon(aes(ymin=최저, ymax=최고), fill="pink", alpha=.7) + geom_line(aes(y=평균), colour="red") + theme_bw()
h + ylab("") + xlab("") + ylim(-12,33)

######################################################### 
# <프로그램 4-8>    우리나라 연도별 경상수지의 추이                       #
######################################################### 

library(ggplot2)
# 데이터 불러오기
cb <- read.csv("i:/R/datavis/cb.csv", header=TRUE)
cb$경상수지 <- cb$경상수지/100
cb$pos <- cb$경상수지 >=0
# 그래프 그리기
...
######################################################### 
# <프로그램 4-9>      연령대별 우리나라 인구 및 인구 비중 추이          #
######################################################### 
...
library(plyr)
# 데이터 불러온 후 연령대별로 재정렬
pop_kr1  <- read.csv("i:/R/datavis/krpop.csv", header=TRUE)
...
library(reshape2)
# 데이터 불러옴
gdp_s1 <- read.csv("i:/R/datavis/gdp_sh.csv", header=TRUE)
# 데이터 불러온 후 재정렬
...
ggplot(gdp_s, aes(x=연도, y=비중, fill=산업))+
  ...
  theme(panel.background = element_rect(fill = "white", colour = "gray"),  legend.position="bottom") + ylab("비중 (%)") + xlab("")

######################################################### 
# <프로그램 4-11>     우리나라 제조업 재고출하순환도의 작성            #
#########################################################
...
library(xts)
# 데이터 불러오기
inven1  <- read.csv("i:/R/datavis/inven_cy.csv",header=TRUE)
...
ggplot(inven_1, aes(x=출하지수증감률, y=재고지수증감률)) + theme_bw() + geom_path() + 
  ...
  geom_line(x=inven_1$출하지수증감률,y=inven_1$출하지수증감률, colour="red") + 
  geom_hline(yintercept=0, colour="gray")+ geom_vline(xintercept=0, colour="gray")

######################################################### 
# <프로그램 4-13>        주가변동의 채색달력 그래프 작성                 #
######################################################### 
